    ...
    if ($ctx -and $ctx.Account) {
        ...
        if ($grantedScopes) {
            ...
            if (-not $missing) {
                ...
                Write-Verbose "Actieve Graph-context gevonden voor account '$($ctx.Account)'."
            }
            else {
                Write-Verbose "Actieve Graph-context mist scopes: $($missing -join ', '). Nieuwe connectie wordt geprobeerd."
            }
        }
    }
